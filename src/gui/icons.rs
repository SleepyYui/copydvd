            ...
            for x in 0..size {
                ...
                let should_draw = match name {
                    "check" => {
                        ...
                        (py - check_y).abs() < 2.0 && px > size as f32 * 0.2 && px < size as f32 * 0.8
                    }
                    "cross" => {
                        // Draw an X
                        let dx = px - center_x;
                    ...
                    "warning" => {
                        ...
                        x_from_center <= triangle_width / 2.0 && py > size as f32 * 0.2 && py < size as f32 * 0.8
                    }
                    "info" => {
                        // Draw an i
                        let distance_from_center = ((px - center_x).powi(2) + (py - center_y).powi(2)).sqrt();
                    ...
                    "search" => {
                        ...
                        let circle_distance = ((px - center_x * 0.8).powi(2) + (py - center_y * 0.8).powi(2)).sqrt();
                        let circle_radius = size as f32 * 0.2;
                        let handle_distance = ((px - center_x * 1.4).powi(2) + (py - center_y * 1.4).powi(2)).sqrt();
                        (circle_distance > circle_radius - 2.0 && circle_distance < circle_radius + 2.0) ||
                        (handle_distance < 3.0 && px > center_x && py > center_y)
                    }
                    "wrench" => {
                        // Draw wrench shape
                        let in_handle = px > size as f32 * 0.1 && px < size as f32 * 0.3 && 
                                       py > size as f32 * 0.3 && py < size as f32 * 0.7;
                        let in_head = ((px - center_x * 1.3).powi(2) + (py - center_y * 0.7).powi(2)).sqrt() < size as f32 * 0.15;
                        in_handle || in_head
                    }
                    "lightning" => {
                        // Draw lightning bolt
                        ...
                        let rel_y = py / size as f32;
                        let in_bolt = (rel_y < 0.5 && rel_x > 0.3 - rel_y * 0.2 && rel_x < 0.7 - rel_y * 0.2) ||
                                     (rel_y >= 0.5 && rel_x > 0.3 + (rel_y - 0.5) * 0.2 && rel_x < 0.7 + (rel_y - 0.5) * 0.2);
                        in_bolt && rel_y > 0.1 && rel_y < 0.9
                    }
                    "lightbulb" => {
                        // Draw light bulb
                        let bulb_distance = ((px - center_x).powi(2) + (py - center_y * 0.8).powi(2)).sqrt();
                        let base_in = px > center_x - size as f32 * 0.1 && px < center_x + size as f32 * 0.1 &&
                                     py > size as f32 * 0.7 && py < size as f32 * 0.9;
                        bulb_distance < size as f32 * 0.25 || base_in
                    }
                    "arrow-right" => {
                        // Draw right arrow
                        let in_shaft = py > center_y - 3.0 && py < center_y + 3.0 && px > size as f32 * 0.2 && px < size as f32 * 0.7;
                        let in_head = (px - center_x * 1.3).abs() + (py - center_y).abs() < size as f32 * 0.15 && px > center_x;
                        in_shaft || in_head
                    }
                    _ => {
                        // Default circle
...
pub fn svg_icon(ui: &mut egui::Ui, name: &str, size: f32, color: Color32) {
    ...
    let color_image = egui::ColorImage::from_rgba_unmultiplied([size as usize, size as usize], &pixels);
...
fn create_icon_pixels_direct(name: &str, size: usize, color: egui::Color32) -> Vec<u8> {
    ...
    for y in 0..size {
        for x in 0..size {
            ...
            let should_draw = match name {
                "check" => {
                    // Draw a beautiful checkmark
                    let t = (px - size as f32 * 0.15) / (size as f32 * 0.7);
                    if (0.0..=1.0).contains(&t) {
                        let check_y = if t < 0.4 {
                            // First part: down stroke
                            center_y + t * size as f32 * 0.4
                        } else {
                            // Second part: up stroke  
                            center_y + (0.4 - (t - 0.4) * 0.8) * size as f32 * 0.4
                        };
                        (py - check_y).abs() < size as f32 * 0.08 && py > center_y - size as f32 * 0.15
                    } else {
                        false
                    }
                }
                ...
                "arrow-right" => {
                    ...
                    let in_shaft = py > center_y - shaft_thickness && py < center_y + shaft_thickness 
                        && px > size as f32 * 0.2 && px < size as f32 * 0.65;
                    
                    // Arrow head as triangle
                    let head_x = px - size as f32 * 0.65;
                    let head_y = py - center_y;
                    let in_head = head_x > 0.0 && head_x < size as f32 * 0.2 
                ...
                "play" => {
                    ...
                    rel_x > 0.0 && rel_x < triangle_width && rel_y > 0.0 && rel_y < triangle_height
                        && rel_y > rel_x * 0.8 && rel_y < triangle_height - rel_x * 0.8
                }
                "pause" => {
                    // Draw pause bars
                    ...
                    let left_bar = px > center_x - spacing - bar_width && px < center_x - spacing
                        && py > start_y && py < start_y + bar_height;
                    let right_bar = px > center_x + spacing && px < center_x + spacing + bar_width
                        && py > start_y && py < start_y + bar_height;
                    left_bar || right_bar
                }
                "stop" => {
                    ...
                    px > start_x && px < start_x + square_size 
                        && py > start_y && py < start_y + square_size
                }
                "folder" => {
                    // Draw folder icon
                    ...
                    // Main folder body
                    let in_body = px > start_x && px < start_x + folder_width
                        && py > start_y + tab_height && py < start_y + folder_height;
                    
                    // Folder tab
                    let in_tab = px > start_x && px < start_x + tab_width
                        && py > start_y && py < start_y + tab_height;
                ...
                "gear" => {
                    ...
                    let effective_radius = if tooth_angle > 0.5 { outer_radius * 1.1 } else { outer_radius * 0.9 };
                ...
                "download" => {
                    ...
                    // Vertical shaft
                    let in_shaft = px > center_x - shaft_width && px < center_x + shaft_width
                        && py > center_y - shaft_height && py < center_y + shaft_height * 0.5;
                    
                    // Down arrow head
                    let arrow_y = py - (center_y + shaft_height * 0.3);
                    let arrow_x = (px - center_x).abs();
                    let in_arrow = arrow_y > 0.0 && arrow_y < arrow_height
                ...
                "upload" => {
                    ...
                    // Vertical shaft
                    let in_shaft = px > center_x - shaft_width && px < center_x + shaft_width
                        && py > center_y - shaft_height * 0.5 && py < center_y + shaft_height;
                ...
                "heart" => {
                    ...
                    let left_circle = ((px - left_center_x).powi(2) + (py - circle_y).powi(2)).sqrt() < heart_size * 0.5;
                    let right_circle = ((px - right_center_x).powi(2) + (py - circle_y).powi(2)).sqrt() < heart_size * 0.5;
                    
                    // Bottom triangle
                    let triangle_tip_y = center_y + heart_size * 0.6;
                    let triangle_x = (px - center_x).abs();
                    let triangle_y_offset = py - circle_y;
                    let in_triangle = py > circle_y && py < triangle_tip_y
                        && triangle_x < heart_size * 0.8 * (1.0 - triangle_y_offset / (heart_size * 0.8));
                ...
                "home" => {
                    ...
                    // House body
                    let in_house = px > start_x && px < start_x + house_width
                        && py > house_y && py < house_y + house_height;
                    
                    // Triangular roof
                    let roof_x_offset = (px - center_x).abs();
                    let roof_y_offset = py - roof_y;
                    let in_roof = roof_y_offset > 0.0 && roof_y_offset < roof_height
                ...
                "menu" => {
                    ...
                    let line1 = px > start_x && px < start_x + line_width
                        && py > center_y - spacing - line_height && py < center_y - spacing;
                    let line2 = px > start_x && px < start_x + line_width
                        && py > center_y - line_height * 0.5 && py < center_y + line_height * 0.5;
                    let line3 = px > start_x && px < start_x + line_width
                        && py > center_y + spacing && py < center_y + spacing + line_height;
                ...
                "save" => {
                    ...
                    // Main disk body
                    let in_body = px > start_x && px < start_x + disk_size
                        && py > start_y && py < start_y + disk_size;
                    
                    // Remove top-right notch
                    let in_notch = px > start_x + disk_size - notch_size && px < start_x + disk_size
                        && py > start_y && py < start_y + notch_size;
                    
                    // Label area
                    let label_height = size as f32 * 0.15;
                    let in_label = px > start_x + size as f32 * 0.1 && px < start_x + disk_size - size as f32 * 0.1
