...
async fn run_cli(app_state: Arc<Mutex<AppState>>, args: cli::Args) -> Result<()> {
    ...
    let tasks = {
        ...
        let output_dir = args.output.unwrap_or_else(|| {
            dvd_lock_guard.config.output_dir.clone()
        });

        let selected_titles_numbers = if args.main_feature {
            info!("Selecting main feature only");
            dvd_lock_guard.find_main_feature().map(|title| vec![title.number])
        ...
        let final_selected_titles = match selected_titles_numbers {
            ...
            Some(_) => { // Empty vec from main_feature if not found
        ...
        let chapter_split = args.chapter_split.unwrap_or(dvd_lock_guard.config.chapter_split);
...

    {
        ...
        state_guard_tasks.status = AppStatus::Ripping { completed: 0, total: task_count };
    } 

    let (tx, mut rx) = mpsc::channel(task_count.max(1));
    let thread_count = {
        let state_guard_threads = app_state.lock().await;
        args.threads.unwrap_or(state_guard_threads.config.thread_count)
    };
    ...
    for (i, task) in tasks.iter().enumerate() {
        ...
        tokio::spawn(async move {
            let result = dvd_clone_task.lock().await.rip_title(&task_clone_exec).await;
            // Send result and then update progress via handle_task_result
            if tx_clone.send((i, result)).await.is_err() {
                warn!("Receiver dropped, could not send task result for task index {}", i);
            }
        });
    ...
    if let AppStatus::Ripping { completed, total } = final_status {
        if completed == total {
            ...
            warn!("Ripping ended but not all tasks reported completion. Status: {:?}", final_status);
        }
    }
    ...
    let (eject_after_rip, server_config_opt) = {
        ...
        (dvd_lock_guard_final.config.eject_after_rip, dvd_lock_guard_final.config.server.clone())
    };
    ...
    if args.upload {
        if let Some(server_config) = server_config_opt {
            ...
            let movie_name = {
                 dvd_arc.lock().await.find_main_feature()
            ...
            for task_item in &tasks { 
                if task_item.output_path.exists() {
                    ...
                    match upload::upload_file(&upload_task).await {
                        ...
                        Err(e) => warn!("Upload failed for {}: {}", task_item.output_path.display(), e),
                    }
                }
            }
        } else {
    ...
    if state_guard_complete.status != AppStatus::Error("".to_string()) { // Avoid overwriting an error state
         if let AppStatus::Ripping{completed, total} = state_guard_complete.status {
            if completed == total {
                state_guard_complete.status = AppStatus::Completed;
            }
         } else if state_guard_complete.status == AppStatus::Idle && task_count == 0 { // No tasks were run
            state_guard_complete.status = AppStatus::Completed;
         }
    }

    info!("CLI run finished. Final status: {:?}", state_guard_complete.status);
    Ok(())
}
