use crate::dvd::types::{Chapter, Dvd, DvdSize, Title};
use crate::error::{AppError, Result};
use regex::Regex;
use std::process::Stdio;
use tokio::process::Command;
use tracing::{info, warn}; // For parsing HandBrake output
    ...
    pub async fn scan_titles(&mut self) -> Result<()> {
        ...
        let output = cmd.output().await.map_err(|e| {
            AppError::HandbrakeError(format!(
                "Failed to execute HandBrakeCLI for scan ({}): {}",
                handbrake_path, e
            ))
        })?;
            ...
            if let Some(json_match) = caps.get(1) {
                ...
                if let Ok(json_val) = serde_json::from_str::<serde_json::Value>(json_str) {
                    if let Some(title_list) = json_val.get("TitleList").and_then(|v| v.as_array()) {
                        self.titles = parse_handbrake_json_titles(title_list)?;
                        info!(
                            "Found {} titles via extracted JSON block.",
                            self.titles.len()
                        );
                        return Ok(());
                    }
                    ...
                    warn!(
                        "Failed to parse extracted JSON from HandBrake scan: {}",
                        json_str
                    );
                }
            }
        ...
        warn!(
            "Could not parse HandBrake scan output as JSON. Stdout: {}",
            stdout
        );
        Err(AppError::HandbrakeError(
            "Failed to parse HandBrakeCLI scan JSON output".to_string(),
        ))
    }
}
